...
private
  ...
  def get_connections
    ...
    @potential_connections_all = Track.all(:select => 'name, id', :order => 'name', :conditions => ["id not in (?)", @track.track_connections.collect(&:connect_track_id) << @track.id])
    @potential_connections_same_area = Track.all(:select => 'name, id', :order => 'name', :conditions => ["id not in (?) AND area_id = ?", @track.track_connections.collect(&:connect_track_id) << @track.id, @track.area_id])
    @potential_connections_same_region = Track.all(:select => 'name, id', :order => 'name', :conditions => ["id not in (?) AND area_id in (?)", @track.track_connections.collect(&:connect_track_id) << @track.id, @track.area.region.areas.collect(&:id)])
  end
